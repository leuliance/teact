...
export function PokemonCard() {
  ...
  const atk = pokemon.stats.find((s) => s.name === 'attack')?.value ?? '?';
  const def = pokemon.stats.find((s) => s.name === 'defense')?.value ?? '?';
  const spd = pokemon.stats.find((s) => s.name === 'speed')?.value ?? '?';
